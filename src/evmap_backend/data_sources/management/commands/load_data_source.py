...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if UpdateMethod.PULL not in data_source.supported_update_methods:
                raise ValueError(
                    f"Data source {source_id} does not support pull updates"
                )
            ...
            self.stdout.write(
                self.style.ERROR(f"Error loading data from {source_id}: {e!s}")
            )
            raise
